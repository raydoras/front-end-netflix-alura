...
export function createCard(item) {
    ...
    const secondaryAction = item.continue ? '<button class="btn-icon"><i class="fas fa-redo"></i></button>' : '<button class="btn-icon"><i class="fas fa-plus"></i></button>';
    details.innerHTML = `
        <div class="details-buttons">
            <div class="left-buttons">
                <button class="btn-icon btn-play-icon"><i class="fas fa-play" style="margin-left:2px;"></i></button>
                ${item.progress ? secondaryAction : '<button class="btn-icon"><i class="fas fa-plus"></i></button>'}
    ...
    card.addEventListener('click', () => {
        const urls = Array.isArray(item.youtube) ? item.youtube : [item.youtube];
        urls.forEach(url => {
